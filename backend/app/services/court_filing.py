...
class CourtFilingService:
    ...
    async def file_case(
        ...
        recommended_court = await self._select_optimal_court(
            case_data, court_preference
        )
    ...
    async def _select_optimal_court(
        self, case_data: Dict[str, Any], preference: Optional[str] = None
    ) -> str:
    ...
    async def _process_filing(
        self, case_id: str, case_data: Dict[str, Any], documents: List[str], court: str
    ) -> Dict[str, Any]:
    ...
    async def track_case(self, case_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_hearing": case["next_hearing"].isoformat()
        ...
        progress_stages = [
            {
                "stage": "Filed",
                "completed": True,
                "date": case["filing_date"].strftime("%Y-%m-%d"),
            },
            {"stage": "Served", "completed": days_since_filing > 7, "date": "Pending"},
            {
                "stage": "Response Due",
                "completed": days_since_filing > 21,
                "date": "Pending",
            },
            {
                "stage": "Case Management",
                "completed": days_since_filing > 35,
                "date": "Pending",
            },
    ...
    async def get_court_recommendations(
        self, case_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        recommendations = {}

        for court_level, court_info in self.courts.items():
            suitability_score = await self._calculate_court_suitability(
                case_data, court_level
            )
    ...
    async def _calculate_court_suitability(
        self, case_data: Dict[str, Any], court_level: str
    ) -> float:
        score = 0.0
        ...
        if court_level == "constitutional" and "constitutional" in case_type:
            score += 9.0
        elif court_level == "high" and (
            "property" in case_type or claim_amount > 400000
        ):
            score += 8.0
    ...
    async def _get_court_pros(
        self, court_level: str, case_data: Dict[str, Any]
    ) -> List[str]:
        pros = {
            "magistrate": [
                "Lower filing fees",
