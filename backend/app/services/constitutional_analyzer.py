...
class ConstitutionalAnalyzerService:
    ...
    async def _analyze_focus_area(
        ...
        if focus_area == "property_rights":
            violations, deduction = await self._analyze_property_rights(law_text, case_context)
        elif focus_area == "administrative_justice":
            violations, deduction = await self._analyze_administrative_justice(law_text, case_context)
        elif focus_area == "access_to_courts":
            violations, deduction = await self._analyze_access_to_courts(law_text, case_context)
        elif focus_area == "public_administration":
            violations, deduction = await self._analyze_public_administration(law_text, case_context)
    ...
    async def _analyze_property_rights(
        ...
        property_indicators = [
            "arbitrary deprivation", "without compensation", "no court oversight",
            "unlawful taking", "administrative seizure", "forced occupation"
        ]
    ...
    async def _analyze_administrative_justice(
        ...
        admin_indicators = [
            "no hearing", "no reasons", "unreasonable", "procedurally unfair",
            "administrative discretion", "no appeal", "arbitrary decision"
        ]
    ...
    async def _analyze_access_to_courts(
        ...
        access_indicators = [
            "no court access", "administrative finality", "no judicial review",
            "ouster clause", "exclusive jurisdiction", "no appeal"
        ]
    ...
    async def _analyze_public_administration(
        ...
        admin_principles = [
            "corruption", "lack of transparency", "no accountability",
            "inefficient", "unethical", "no public participation"
        ]
    ...
    async def _generate_challenge_strategy(self, violations: List[Dict]) -> Dict[str, Any]:
        if not violations:
        ...
        strategy = {
            ...
            "legal_arguments": {
                ...
                "remedies_sought": list(set([remedy for v in violations for remedy in v["remedies"]]))
            },
            "court_strategy": {
                "recommended_court": "Constitutional Court" if len(primary_violations) > 2 else "High Court",
                "urgency_level": "Urgent" if any(v["severity_score"] >= 9 for v in violations) else "Standard",
