...
class ConstitutionalViolationDetector:
    ...
    async def scan_systematic_violations(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_priority_violations": [v for v in all_violations if v["priority"] >= 8],
            "challenge_ready_violations": [v for v in all_violations if v["challenge_ready"]],
            "public_dashboard_data": await self._generate_dashboard_data(all_violations),
    ...
    async def _analyze_law_for_violations(self, law: Dict[str, str], category: str) -> Dict[str, Any]:
        ...
        if constitutional_analysis.get("violations_detected", False):
            return {
                ...
                "constitutional_sections_violated": constitutional_analysis["violated_sections"],
                "description": constitutional_analysis["violation_description"],
                "legal_analysis": constitutional_analysis["detailed_analysis"],
                "precedent_cases": constitutional_analysis.get("relevant_precedents", []),
                "priority": constitutional_analysis["challenge_priority"],
                "success_probability": constitutional_analysis["challenge_success_probability"],
                "challenge_ready": constitutional_analysis["challenge_success_probability"] > 0.7,
                "estimated_impact": constitutional_analysis["potential_impact"],
                "affected_population": constitutional_analysis.get("affected_population", "Unknown"),
    ...
    async def _generate_dashboard_data(self, violations: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "violations_by_constitutional_section": self._group_by_constitutional_section(valid_violations),
            "challenge_ready_count": len([v for v in valid_violations if v.get("challenge_ready")]),
            "high_impact_violations": [v for v in valid_violations if "high" in v.get("estimated_impact", "").lower()],
            "recent_detections": sorted(valid_violations, key=lambda x: x["detection_timestamp"], reverse=True)[:10]
        }
    ...
    def _group_by_constitutional_section(self, violations: List[Dict[str, Any]]) -> Dict[str, int]:
        section_counts = {}
        for violation in violations:
    ...
    async def generate_challenge_for_violation(self, violation_id: str) -> Dict[str, Any]:
        ...
        challenge_document = await self.copilot.generate_legal_document(
            ...
            case_details={
                ...
                "constitutional_sections": violation["constitutional_sections_violated"],
    ...
    async def _generate_challenge_strategy(self, violation: Dict[str, Any]) -> List[str]:
        return [
    ...
    async def _estimate_challenge_resources(self, violation: Dict[str, Any]) -> Dict[str, Any]:
        return {
