...
from app.auth.security import (create_access_token, get_current_user,
                               get_password_hash, require_role, require_roles,
                               verify_password)
...
from app.models.legal_governance import (DocumentTemplate, LegalReviewer,
                                         ReviewStatus, TemplateApproval)
from app.models.schemas import (CaseData, ConstitutionalAnalysisRequest,
                                CourtFilingRequest, DocumentRequest,
                                DocumentResponse, LawyerAccountabilityRequest)
...
async def generate_document(
    ...
    try:
        high_risk_templates = [
            ...
            "asset_preservation_order",
            "corruption_report",
            "trust_accounting_demand",
        ]

        if request.document_type in high_risk_templates:
            template = (
                db.query(DocumentTemplate)
                .filter(DocumentTemplate.template_name == request.document_type)
                .first()
            )
...
async def get_document_types():
    return {
        "existing_plebeian": [
            ...
            "bill_of_exchange",
            "fraud_notice",
            "birth_certificate_app",
            "trust_accounting_demand",
            "settlement_demand",
            "retroactive_claim",
            "debt_collector_challenge",
            "mortgage_disclosure_demand",
        ],
        "property_rights": [
            "urgent_eviction_application",
            "standard_eviction_application",
            "criminal_charges_police",
            "criminal_charges_prosecutor",
            "criminal_charges_court",
            "constitutional_challenge",
...
async def get_template_approval_status(
    current_user: User = Depends(
        require_roles([UserRole.REVIEWER, UserRole.LAWYER, UserRole.ADMIN])
    ),
    db: Session = Depends(get_db),
):
    ...
    return [
        {
            ...
            "approval_count": len(
                [a for a in template.approvals if a.status == ReviewStatus.APPROVED]
            ),
        }
...
async def approve_template(
    ...
    reviewer = (
        db.query(LegalReviewer)
        .filter(
            LegalReviewer.user_id == current_user.id, LegalReviewer.is_active == True
        )
        .first()
    )
