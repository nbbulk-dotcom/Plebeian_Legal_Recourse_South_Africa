...
def upgrade() -> None:
    ...
    sa.Column('role', sa.Enum('CITIZEN', 'REVIEWER', 'LAWYER', 'ADMIN', name='userrole'), nullable=False),
    ...
    op.create_table('legal_reviewers',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    op.create_index(op.f('ix_legal_reviewers_id'), 'legal_reviewers', ['id'], unique=False)
    ...
    sa.Column('status', sa.Enum('PENDING', 'APPROVED', 'REJECTED', 'REQUIRES_REVISION', name='reviewstatus'), nullable=False),
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.ForeignKeyConstraint(['reviewer_id'], ['legal_reviewers.id'], ),
    ...
    sa.Column('violation_type', sa.Enum('PROPERTY_RIGHTS', 'ADMINISTRATIVE_JUSTICE', 'EQUALITY', 'HUMAN_DIGNITY', 'FREEDOM_OF_EXPRESSION', 'ACCESS_TO_COURTS', 'JUST_ADMINISTRATIVE_ACTION', name='violationtype'), nullable=False),
    ...
    op.create_index(op.f('ix_constitutional_violations_id'), 'constitutional_violations', ['id'], unique=False)

    op.create_table('lawyer_accountability',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    op.create_index(op.f('ix_lawyer_accountability_id'), 'lawyer_accountability', ['id'], unique=False)
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
    sa.ForeignKeyConstraint(['lawyer_id'], ['lawyer_accountability.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_lawyer_accountability_id'), table_name='lawyer_accountability')
    op.drop_table('lawyer_accountability')
    op.drop_index(op.f('ix_constitutional_violations_id'), table_name='constitutional_violations')
